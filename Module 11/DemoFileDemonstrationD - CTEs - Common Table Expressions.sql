...
-- Step 2: Common Table Expressions (CTEs). Select this query and execute it to show CTE examples.
...
WITH EmpOrg_CTE AS
	(
		SELECT
			empid, mgrid, lastname, firstname -- anchor query
		FROM HR.Employees
		...
		SELECT
			child.empid, child.mgrid, child.lastname, child.firstname -- recursive member which refers back to CTE
