...
-- Step 2: Set up for demo. Clean up if the procedure already exists.

IF OBJECT_ID('Production.ProdsByCategory', 'P') IS NOT NULL
	DROP PROC Production.ProdsByCategory;
GO

-- Create a proc to search for category and a parameter to limit the number of results.
...
-- Step 3: Exploring batches. Execute the following batch to show statements submitted and executed as one unit.
-- Valid batch.
...
-- Step 5: Batches with errors. Run the following batch to show entire batch rejected on compile/syntax error.
...
-- Step 6: Variables. Show the following code to illustrate use of variables for executing stored procedure.
...
-- Run the following batch in its entirety to show the choices for assigning values to variables.
...
SELECT
	@var1 AS var1, @var2 AS var2, @var3 AS var3;
GO

-- Step 7: Synonyms. Clean up if the procedure already exits.

IF OBJECT_ID('Production.ProdsByCategory', 'P') IS NOT NULL
	DROP PROC Production.ProdsByCategory;
GO

-- Declare a parameter to search for category and a parameter to limit the number of results.
