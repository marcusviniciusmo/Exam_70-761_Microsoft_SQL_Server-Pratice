...
/*
	When first connecting to an Azure Database server
	you must use the Available Databases drop down and select ADVENTUREWORKSLT.
...
/*
	...
	and that are 542 rows returned.
*/
...
/*
	...
	Column 'SalesLt.SalesOrderDetail.SalesOrderID' is invalid in the select list
...
/*
	Select and run the partial query to show results.
	Point out that the * in the SELECT list has been replaced
	with columns that are either in the GROUP BY expression or are aggregate functions
	(this will be explained further in Module 9).
...
/*
	Select and run the partial query to show results.
	Point out that a HAVING clause further filters the results based on the groups,
	and also that there are 8 rows returned and  there is no apparent sort order.
*/
